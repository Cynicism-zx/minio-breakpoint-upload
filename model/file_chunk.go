...
func Init() {
	if !postgres.Global.DB.HasTable(&FileChunk{}) {
		postgres.Global.DB.CreateTable(&FileChunk{})
	}
	postgres.Global.DB.AutoMigrate(&FileChunk{})
}

// GetFileChunkByMD5 returns fileChunk by given md5
func GetFileChunkByMD5(md5 string) (*FileChunk, error) {
	fileChunk := new(FileChunk)
	if err := postgres.Global.DB.Where("md5 = ?", md5).Find(&fileChunk).Error; err != nil {
...
func GetFileChunkByUUID(uuid string) (*FileChunk, error) {
	fileChunk := new(FileChunk)
	if err := postgres.Global.DB.Where("uuid = ?", uuid).Find(&fileChunk).Error; err != nil {
		return fileChunk, err
	}
	return fileChunk, nil
}
...
func InsertFileChunk(fileChunk *FileChunk) (_ *FileChunk, err error) {
	if err := postgres.Global.DB.Create(fileChunk).Error; err != nil {
		return fileChunk, err
	}
	return fileChunk, nil
}
...
func UpdateFileChunk(fileChunk *FileChunk) error {
	if err := postgres.Global.DB.Model(&fileChunk).Where("uuid = ?", fileChunk.UUID).
